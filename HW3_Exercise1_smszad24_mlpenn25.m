...
k_close = -72; % the potassium gate closes at this value
...
dvdt = @(I_ext,V,n,m,h,open_k,open_na) (I_ext-Ik(V,n,m,h,open_k)...
    -Ina(V,n,m,h,open_na)-Il(V,n,m,h))/capacitance;
...
xlabel("time (ms)")
legend("n","m","h")
title("Gating constants of Hodgkin-Huxley Model without Na-K Pump")

% graphing the action potential with respect to time
figure
plot(0:time_step:simulation_length, V_vals)
xlabel("time (ms)")
title("Action Potential of Hodgkin-Huxley Model without Na-K Pump")

...
Vk = -75; % displacement from the equilibrium potential for potassium
...
na_close = 49.3; % the sodium gate closes at this value
k_open = 49.3; % the potassium gate opens at this value
k_close = -74; % the potassium gate closes at this value
...
for i = null_steps:total_steps+1
    ...
    deltaV_1 = dvdt(I_ext,V_vals(i-1),n_vals(i-1),m_vals(i-1),h_vals(i-1), ...
        open_val_k, open_val_na)* time_step; 
...
xlabel("time (ms)")
legend("n","m","h")
title("Gating constants of Hodgkin-Huxley Model with Na-K Pump")

% graphing the action potential with respect to time
figure
plot(0:time_step:simulation_length, V_vals)
xlabel("time (ms)")
title("Action Potential of Hodgkin-Huxley Model with Na-K Pump")

...
Il = @(V,n,m,h,open_l) open_l*gl*(V-Vl);
Ip = Il(init_V,init_n,init_m,init_h,open_val_l);
...
dvdt = @(I_ext,V,n,m,h,open_k,open_na,open_l) (I_ext-Ik(V,n,m,h,open_k)...
    -Ina(V,n,m,h,open_na)-(Il(V,n,m,h,open_l)-Ip))/capacitance;
...
for i = null_steps:total_steps+1
    ...
    % when the voltage reaches -54.4 mV, the leakage gate closes,
    % symbolized by the open_val_l switching from 1 to 0
    if V_vals(i-1) <= l_close
        open_val_l = 0;
    end
    ...
    if Nai > 0 && Ko > 0
        ...
        Ip = Il(init_V,init_n,init_m,init_h,open_val_l); % setting the pump 
        % constant
    else
    ...
    deltaV_1 = dvdt(I_ext,V_vals(i-1),n_vals(i-1),m_vals(i-1),h_vals(i-1),... 
    open_val_k, open_val_na, open_val_l)*time_step; 
    ...
    deltaV_2 = dvdt(I_ext,V_vals(i-1)+(deltaV_1*0.5),n_vals(i-1)+(deltan_1*0.5), ...
        ...
        open_val_na, open_val_l)* time_step; 

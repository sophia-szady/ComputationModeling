% Final Project 5: Simulation of a 1 cell animal moving through the 
% simulation eating mushrooms depending on its neighboring, using a Moore
% neighborhood and 2 randomly set spores for mushroom growth with 
...
% This simulation is the second version of the animal that now moves 
% based on its neighbors, specifically in the directions of mushrooms
% to simulate it eating
...
% initializing a random coordinates for the initial spores
initSporeX1 = randi(r);
initSporeY1 = randi(c);
initSporeX2 = randi(r);
initSporeY2 = randi(c);

% setting the animal to be in the upper middle of the field
initAnimal = animal;
initField(r-10,50) = initAnimal;

% setting the position of the initial spores in the field and extField
...
% an anonymous function to get the current indices of the neighbors, in the
% same order as the mooreNeighborVals to correspond to the same neighbor
mooreNeighborsIndxs = @(currentR, currentC) [[currentR+1 currentC]...
    [currentR+1 currentC+1] [currentR currentC+1] ...
    [currentR-1 currentC-1] [currentR-1 currentC]...
    [currentR-1 currentC-1] [currentR currentC-1] ...
...
for i = 2:numIterations % starting after the initial field setup
    ...
    for m = 1:r % going through each row in the field
        for n= 1:c % going through each column in the field
            ...
            if current==animal  
                if ismember(mushrooms,neighborList) % if any of the
                    % neighbors are mushrooms
                    firstMushroom=find(neighborList==mushrooms,1); % find 
                    % the first occurence of mushrooms in neighborList
                    animalNeighbors=mooreNeighborsIndxs(m,n); % get a list
                    % of the indices of all the neighbors
                    firstMushroomR = animalNeighbors(firstMushroom*2-1); 
                    % finding the x index of the first neighbor that is a
                    % mushroom
                    firstMushroomC=animalNeighbors(firstMushroom*2); 
                    % finding the x index of the first neighbor that is a
                    % mushroom
                    if firstMushroomR<r && firstMushroomR>0 && ...
                            firstMushroomC<c && firstMushroomC>0
                        % if the mushroom location is not one of the
                        % boundary cells then the animal moves to that cell
                        field(firstMushroomR,firstMushroomC) = animal;
                    end
                    % if not the cell become empty
                    field(m,n)=empty;
                end  
